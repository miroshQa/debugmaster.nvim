...
M.sidepanel:set_on_active_callback(function(buf)
  ...
  vim.keymap.set("n", "H", function()
    M.sidepanel:toggle_active_with_open(M.help)
    mode.activate()
  end, { buffer = buf })
  vim.keymap.set("n", "S", function()
    M.sidepanel:toggle_active_with_open(M.scopes)
    mode.activate()
  end, { buffer = buf })
  vim.keymap.set("n", "R", function()
    mode.activate()
    M.sidepanel:toggle_active_with_open(M.repl)
  end, { buffer = buf })
  vim.keymap.set("n", "P", function()
    M.sidepanel:toggle_active_with_open(M.terminal)
    mode.activate()
  end, { buffer = buf })
end)
